subroutine psb_dscg_vect(a, prec, b, x, s, eps, desc_a, info, &
  ...
  do itidx = 1, itmax_
    ...
    select case(Gram_solver_)
      ...
      case(lapackCC);   call dpotrs('L', s, 1, W, s, alpha, s, linfo)
      case default
        info = psb_err_invalid_input_ 
        call psb_errpush(info, name)
        goto 9999
    ...
    select case(Gram_solver_)
      ...
      case(lapackCC);   call dpotrs('L', s, s, W, s, beta, s, linfo)
      case default
        info = psb_err_invalid_input_ 
        call psb_errpush(info, name)
        goto 9999
    ...
    ! of dgemm, so Z = Z + P*beta computes the new direction block in place;
...
contains
  ...
  subroutine swap_mv(x, y)
    ...
    class(psb_d_base_multivect_type), allocatable :: t
...
subroutine psb_dscg2_vect(a, prec, b, x, s, eps, desc_a, info, &
  ...
  integer(psb_ipk_)   :: err_act, np, me, debug_level, debug_unit, &
                          & n_col, n_row
  integer(psb_lpk_)   :: mglob
  character(len=20)           :: name = 'psb_dscg2'
