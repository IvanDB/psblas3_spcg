subroutine psb_cscg_vect(a, prec, b, x, s, eps, desc_a, info, &
  ...
  type(psb_c_multivect_type)  :: Z, Q, P, V
  ...
  do itidx = 1, itmax_
    ...
    linfo = 0
    if(Gram_solver_ == lapackLU) call cgetrf(s, s, W, s, pW, linfo)
    if(Gram_solver_ == lapackCC) call cpotrf('L', s, W, s, linfo)
    if(linfo /= 0) then
      info = psb_err_from_subroutine_
    ...
    select case(Gram_solver_)
      ...
      case(lapackLU);   call cgetrs('N', s, 1, W, s, pW, alpha, s, linfo)
      case(lapackCC);   call cpotrs('L', s, 1, W, s, alpha, s, linfo)
      case default
        info = psb_err_invalid_input_ 
        call psb_errpush(info, name)
        goto 9999
    ...
    select case(Gram_solver_)
      ...
      case(lapackLU);   call cgetrs('N', s, s, W, s, pW, beta, s, linfo)
      case(lapackCC);   call cpotrs('L', s, s, W, s, beta, s, linfo)
      case default
        info = psb_err_invalid_input_ 
        call psb_errpush(info, name)
        goto 9999
...
contains
  ...
  subroutine swap_mv(x, y)
    ...
    class(psb_c_base_multivect_type), allocatable :: t
...
subroutine psb_cscg2_vect(a, prec, b, x, s, eps, desc_a, info, &
  ...
  integer(psb_ipk_)   :: err_act, np, me, debug_level, debug_unit, &
                          & n_col, n_row
  integer(psb_lpk_)   :: mglob
  character(len=20)           :: name = 'psb_cscg2'
